...
from mtrag.llava.constants import IGNORE_INDEX, IMAGE_TOKEN_INDEX, DEFAULT_IMAGE_TOKEN, DEFAULT_IM_START_TOKEN, DEFAULT_IM_END_TOKEN, DEFAULT_REGION_FEA_TOKEN
...
def parse_args(args):
    parser = argparse.ArgumentParser(description="MTRAG Inference - RefCOCOs")
    ...
    parser.add_argument("--exp_name", default="mtrag_eval_referseg", type=str)
...
def initialize_datasets_and_loaders(args, tokenizer):
    ...
    val_datasets = [ReferExpSegmDataset(**common_ds_args, validation=True, refer_segm_data=dataset, split=split, inference=True)]
...
def evaluate_model_performance(validation_loader, model, args):
    ...
    for data_batch in tqdm.tqdm(validation_loader):
        ...
        for target, prediction in zip(gt_masks, predicted_masks):
            ...
            union += union_
            accuracy_iou_list.append(intersect / (union_ + 1e-5))
