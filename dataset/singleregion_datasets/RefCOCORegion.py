...
class RefRegDataset(torch.utils.data.Dataset):
    CLASSES = ('object',)
    ...
    def __init__(self, dataset_dir, tokenizer, global_image_encoder, precision="fp32",
                 ...
                 image_dir='', json_path='', intro_string='', question_templates=None, random_sampling=True, use_mm_start_end=False, **kwargs):
...
class RefCocoRegDataset(RefRegDataset):
    def __init__(self, dataset_dir, tokenizer, global_image_encoder, precision="fp32",
        ...
        intro_string = DEFAULT_IMAGE_TOKEN + "\n" + ("I will provide you with only one region containing only one "
                                                     ...
                                                     "as well as its position within the image and its basic attributes.")
        json_files = {'validation': "finetune_refcoco_val_with_mask.json", 'training': "finetune_refcoco_train_with_mask.json"}
        json_path =  json_files['validation'] if validation else json_files['training']
        dataset_name = "region_cap"
        ...
        super().__init__(
            ...
            intro_string, question_templates, random_sampling,use_mm_start_end, **kwargs
            )
        print('\033[92m' + "----REGION-{}: Loaded RefCOCO dataset ----".format(mode) + '\033[0m')


class RefCocoGRegDataset(RefRegDataset):
    def __init__(self, dataset_dir, tokenizer, global_image_encoder, precision="fp32",
                 image_size=1024, num_classes_per_sample=3, max_gt_per_img=10, validation=False, random_sampling=True, use_mm_start_end=False, **kwargs):
        intro_string = f""" {DEFAULT_IMAGE_TOKEN}\nThis provides an overview of the picture.\n"""
        dataset_name = "region_cap"
        json_files = {'validation': "finetune_refcocog_val_with_mask.json", 'training': "finetune_refcocog_train_with_mask.json"}
        json_path = json_files['validation'] if validation else json_files['training']
        image_dir = "coco_2014/train2014"
        # question_templates = REGION_QUESTIONS
        question_templates = ["Please give me a short description of <region>",]
        mode = "Val" if validation else "Train"
...
class RefCocoPRegDataset(RefRegDataset):
    def __init__(self, dataset_dir, tokenizer, global_image_encoder, precision="fp32",
                 image_size=1024, num_classes_per_sample=3, max_gt_per_img=10, validation=False, random_sampling=True, use_mm_start_end=False, **kwargs):
        intro_string = DEFAULT_IMAGE_TOKEN + "\n" + ("I will provide you with only one region containing only one "
                                                     "object, although there may be other objects present in the "
                                                     ...
                                                     "attributes.")
        dataset_name = "region_cap"
        json_files = {'validation': "finetune_refcoco+_val_with_mask.json", 'training': "finetune_refcoco+_train_with_mask.json"}
        json_path = json_files['validation'] if validation else json_files['training']
        image_dir = "coco_2014/train2014"
        question_templates = ['<region>', ]
        mode = "Val" if validation else "Train"
