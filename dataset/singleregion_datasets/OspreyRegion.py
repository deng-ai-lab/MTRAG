...
class OspreyBaseDataset(torch.utils.data.Dataset):
    IMG_MEAN = torch.Tensor([123.675, 116.28, 103.53]).view(-1, 1, 1)
    IMG_STD = torch.Tensor([58.395, 57.12, 57.375]).view(-1, 1, 1)
    IMG_SIZE = 1024
    IGNORE_LABEL = 255
    def __init__(self, dataset_dir, tokenizer, global_image_encoder, precision="fp32",
                 image_size=1024, num_classes_per_sample=3, max_gt_per_img=10, validation=False, dataset_name='', image_dir='', json_path="", limit_str="",random_sampling=True, use_mm_start_end=False, **kwargs):
    ...
    def _parse_annotations(self, ann):
        ...
        for i in range(region_num):
            ...
            if i>0:
                str_region+= ','
            str_region += "region"+str(i+1)+" <region_fea>"
        for i in range(len(ann['conversations'])//2):
            if i==0:
                ...
                question = re.sub(r'<regin(\d+)>', lambda match: f'region{match.group(1)}', question) # for partially incorrect data:<regin1>
                question = self.begin_str+mid_str+question
                conversations['conversations'].append({'from': 'human', 'value': question+self.limit})         
            else:
                question = ann['conversations'][i*2]['value']
                question = re.sub(r'<region(\d+)>', lambda match: f'region{match.group(1)}', question)
                question = re.sub(r'<regin(\d+)>', lambda match: f'region{match.group(1)}', question) # for partially incorrect data:<regin1>
            ...
            answer = re.sub(r'<regin(\d+)>', lambda match: f'region{match.group(1)}', answer) # for partially incorrect data:<regin1>
    ...
    def __getitem__(self, index):
        while True:
            ...
            if not len(ann['conversations'])//2 ==0:
                data_info = self._parse_annotations(ann)
                if min(data_info['width'], data_info['height']) >= 32 and len(data_info['conversations']['conversations'])>0:
                    break
...
class OspreyConversations(OspreyBaseDataset):
    def __init__(self, dataset_dir, tokenizer, global_image_encoder, precision="fp32",
        ...
        limit_str = ""
        json_path = "osprey_conversation.json"
        dataset_name = "osprey_instruct"
        ...
        super().__init__(
            ...
            limit_str, random_sampling, use_mm_start_end, **kwargs)
        print('\033[92m' + "----REGION-{}: Loaded OspreyConversations dataset ----".format(mode) + '\033[0m')


class OspreyDetailedDescription(OspreyBaseDataset):
    def __init__(self, dataset_dir, tokenizer, global_image_encoder,  precision="fp32",
                 image_size=1024, num_classes_per_sample=3, max_gt_per_img=10, validation=False, random_sampling=True, use_mm_start_end=False, **kwargs):
        limit_str = ""
        json_path = "osprey_detail_description.json"
        dataset_name = "osprey_instruct"
        image_dir = "coco_2014/train2014"
        mode = "Val" if validation else "Train"
        super().__init__(
            ...
            max_gt_per_img, validation, dataset_name, image_dir, json_path,
            limit_str, random_sampling, use_mm_start_end, **kwargs)
        print('\033[92m' + "----REGION-{}: Loaded OspreyDetailedDescription dataset ----".format(mode) + '\033[0m')
    def _parse_annotations(self, ann):
        masks = []
        conversations = dict()
        conversations['conversations'] = []
        ...
        for i in range(region_num):
            ...
            question = question.replace('<region>', f'region{i+1} <region_fea>')
            if i==0:
...
class OspreyPartLevel(OspreyBaseDataset):
    def __init__(self, dataset_dir, tokenizer, global_image_encoder, precision="fp32",
                 image_size=1024, num_classes_per_sample=3, max_gt_per_img=10, validation=False, random_sampling=True, use_mm_start_end=False, **kwargs):
        limit_str = "Answer the question using a single word or phrase."
        json_path = "osprey_part_level.json"
        dataset_name = "osprey_instruct"
        ...
        super().__init__(
            ...
            max_gt_per_img, validation, dataset_name, image_dir, json_path,
            limit_str, random_sampling, use_mm_start_end, **kwargs)
        print('\033[92m' + "----REGION-{}: Loaded OspreyPartLevel dataset ----".format(mode) + '\033[0m')