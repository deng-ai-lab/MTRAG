...
class RIOMultiRegDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, dataset_dir, tokenizer, global_image_encoder, precision="fp32",
        ...
        self.data_infos = [sample for sample in self.data_infos if len(sample['mask_list'])> 1 and min(sample['width'], sample['height']) >= 32]
    ...
    def _parse_annotations(self, ann):
        ...
        question = "What can I do with the {region}?"
        answer = ann['expressions'][0]
        segm_list = ann['mask_list']
        region_string = ''
        for i in range(len(segm_list)):
            mask = self.annToMask(segm_list[i], h, w)
            masks.append(mask)
            region_string += f"region{i + 1} <region_fea>"
            if i < len(segm_list)-2:
                region_string += ', '
            elif i == len(segm_list) - 2:
                region_string += ' and '
        question = self.begin_str + question.format(region=region_string)
...
class RIOTrainMultiRegDataset(RIOMultiRegDataset):
    def __init__(self, dataset_dir, tokenizer, global_image_encoder, precision="fp32",
        ...
        print('\033[92m' + "----RIOTrainMultiReg-{}: RIOTrainMultiReg dataset initialized----".format(mode) + '\033[0m')