...
class LLaVAInstructDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, dataset_dir, tokenizer, global_image_encoder, epoch_samples=10000, precision="fp32",
        ...
        self.image_folder = dataset_dir
        annotations_file = os.path.join(self.base_dir, "llava_v1_5_mix665k.json")
        self.data_infos = self._load_annotations(annotations_file)
        self.grounding = kwargs.get("grounding", True)
        print('\033[92m' + "----CAP----: LLaVA-Instruct dataset initialized----".format(mode) + '\033[0m')
    @property
    def lengths(self):
        length_list = []
    ...
    def __getitem__(self, idx):
        ...
        if ann_info.get("image", None) is None:
            image = None
            image_path = ''
            crop_size = self.global_enc_processor.crop_size
            image = None
            global_enc_image = torch.zeros(3, crop_size['height'], crop_size['width'])
            grounding_enc_image = torch.zeros(3, self.IMG_SIZE, self.IMG_SIZE)
            region_masks = None
            masks = None
            label = None
            image_resize = None
